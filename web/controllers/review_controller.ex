defmodule SymphonyApi.ReviewController do
  use SymphonyApi.Web, :controller
  ...
  def create(conn, %{"review" => review_params}) do
    ...
    case Repo.insert(changeset) do
      {:ok, review} ->
        conn
        |> put_status(:created)
        |> put_resp_header("location", venue_review_path(conn, :show, review))
        |> render("show.json", review: review)
      {:error, changeset} ->
        conn
        |> put_status(:unprocessable_entity)
        |> render(SymphonyApi.ChangesetView, "error.json", changeset: changeset)
    end
  end
